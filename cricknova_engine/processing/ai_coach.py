...
def _normalize_plan(raw_plan) -> str:
    ...
    if not plan:
        return "FREE"
    alias = PLAN_ALIASES.get(plan.lower())
    if alias:
        return alias
    return plan.upper()
...
def _is_active_subscription(sub: dict) -> tuple[bool, str, datetime | None]:
    plan = _normalize_plan(sub.get("plan") or sub.get("plan_id"))
    premium_flag = sub.get("isPremium") is True or sub.get("premium") is True
    expiry = _parse_expiry(
        sub.get("expiry") or sub.get("expiryDate") or sub.get("expiry_date")
    )
    if plan == "FREE":
...
def get_user_plan(user_id: str) -> str:
    ...
    try:
        ...
        active, plan, expiry = _is_active_subscription(data)
        print("🧾 SUBSCRIPTION DATA:", {"user_id": user_id, "plan": plan, "expiry": expiry})
        return plan if active else "FREE"
...
def check_limit(user_id: str, feature: str):
    ...
    active, raw_plan, expiry = _is_active_subscription(sub)
    if not active:
        return False, True, {
    ...
    if used >= limit:
        print("⚠️ Premium usage counter reached, but allowing reply for active subscriber")
        return True, False, {
            "used": used,
            "limit": limit,
            "remaining": 0
...
async def ai_coach(
    ...
    if plan != "FREE":
        ...
        # Keep free users responsive too, but tag the response for analytics.
        skip_limit = True
