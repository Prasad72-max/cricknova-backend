...
import 'cricknova_mistake_analysis_step.dart';
...
class _CricknovaPrePaywallFlowScreenState
  ...
  _PrePaywallStep _step = _PrePaywallStep.comparison;
  ...
  int get _index {
    ...
    if (_includeSnapshot) {
      return switch (_step) {
        ...
        _PrePaywallStep.mistakeAnalysis => 5,
        _PrePaywallStep.trial => 6,
    ...
    return switch (_step) {
      ...
      _PrePaywallStep.mistakeAnalysis => 4,
  ...
  void _next() {
    setState(() {
      _step = switch (_step) {
        ...
        _PrePaywallStep.comparison => _PrePaywallStep.mistakeAnalysis,
        _PrePaywallStep.snapshot => _PrePaywallStep.mistakeAnalysis,
        _PrePaywallStep.hook => _PrePaywallStep.mistakeAnalysis,
        _PrePaywallStep.trust => _PrePaywallStep.mistakeAnalysis,
        _PrePaywallStep.mistakeAnalysis => _PrePaywallStep.trial,
  ...
  void _back() {
    if (_step == _PrePaywallStep.login ||
        ...
        _step == _PrePaywallStep.snapshot ||
    ...
    setState(() {
      _step = switch (_step) {
        ...
        _PrePaywallStep.mistakeAnalysis => _PrePaywallStep.comparison,
        _PrePaywallStep.trial => _PrePaywallStep.mistakeAnalysis,
      };
    });
    HapticFeedback.selectionClick();
  }
  ...
  Future<void> _openPaywall() async {
    ...
    await _launchSelectedCheckout();
  }

  Future<void> _launchSelectedCheckout() async {
    ...
    try {
      ...
      final bool wantsYearly = _plan == _PlanChoice.yearly;
      ...
      if (selectedPlan == null && allowTrial) {
      ...
      final bool shouldUseTrial = wantsYearly && selectedPlan.hasFreeTrial;
      final launched = await subscriptionProvider.purchasePlan(
        selectedPlan,
        allowFreeTrial: shouldUseTrial,
        requireFreeTrial: shouldUseTrial,
      );
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isTrialStep = _step == _PrePaywallStep.trial;
                  ...
                  Expanded(
                    child: AnimatedSwitcher(
                      ...
                      child: switch (_step) {
                        ...
                        _PrePaywallStep.mistakeAnalysis =>
                          CricknovaMistakeAnalysisStep(
                            key: stepKey,
                            teal: _teal,
...
class _TrialStep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool yearlySelected = selectedPlan == _PlanChoice.yearly;
    final String yearlyPrice = isIndia ? '₹499 / year' : '\$59.99 / year';
    final String monthlyPrice = isIndia ? '₹99 / month' : '\$8.99 / month';
    final String yearlyFooterCopy = isIndia
        ? '3 days free, then ₹499/year (₹41/mo)'
        : '3 days free, then \$59.99/year (\$5/mo)';
    final String monthlyFooterCopy = isIndia
        ? 'Monthly access starts now at ₹99/month.'
        : 'Monthly access starts now at \$8.99/month.';
    final String footerCopy = yearlySelected
        ? yearlyFooterCopy
        : monthlyFooterCopy;
    ...
    final String billingLabel =
        "You'll be charged on ${months[bd.month - 1]} ${bd.day}, ${bd.year} unless you cancel anytime before.";
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final veryCompact = constraints.maxHeight < 690;
          final horizontalPadding = veryCompact ? 18.0 : 22.0;
          return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (yearlySelected) ...[
                  SizedBox(height: veryCompact ? 10 : 14),
                  _CalAITimeline(billingLabel: billingLabel, compact: compact),
                  SizedBox(height: veryCompact ? 10 : 14),
                ] else
                ...
                _TrialPlanCard(
                  ...
                  selected: selectedPlan == _PlanChoice.monthly,
                  showBadge: false,
                  compact: compact,
                  onTap: () => onPlanChange(_PlanChoice.monthly),
                ),
                SizedBox(height: veryCompact ? 10 : 12),
                _TrialPlanCard(
                  title: 'Yearly',
                  priceLabel: yearlyPrice,
                  selected: selectedPlan == _PlanChoice.yearly,
                  showBadge: true,
                  compact: compact,
                  onTap: () => onPlanChange(_PlanChoice.yearly),
                ),
                SizedBox(height: veryCompact ? 10 : 14),
                if (yearlySelected) ...[
                  _NoPaymentLine(compact: compact),
                  SizedBox(height: veryCompact ? 10 : 12),
                ],
                const Spacer(),
                SizedBox(
                  width: double.infinity,
                  height: veryCompact ? 50 : 54,
                ...
                Text(
                  ...
                  style: GoogleFonts.inter(
                    ...
                    fontSize: veryCompact ? 11.5 : 12.5,
                    fontWeight: FontWeight.w700,
                  ),
                ),
                if (yearlySelected) ...[
...
class _CalAITimeline extends StatelessWidget {
  ...
  const _CalAITimeline({required this.billingLabel, required this.compact});
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        Column(
          children: [
            Container(
              ...
              child: Icon(iconData, color: iconColor, size: compact ? 16 : 18),
            ),
            if (!isLast)
              Container(
                width: 2.5,
        ...
        Expanded(
          child: Padding(
            padding: EdgeInsets.only(top: 1, bottom: compact ? 6 : 8),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(
        ...
        Material(
          ...
          child: InkWell(
            ...
            child: AnimatedContainer(
              ...
              curve: Curves.easeOut,
              width: double.infinity,
              constraints: BoxConstraints(minHeight: compact ? 72 : 82),
              padding: EdgeInsets.fromLTRB(
                compact ? 16 : 18,
                compact ? 10 : 12,
                compact ? 14 : 16,
...
class _NoPaymentLine extends StatelessWidget {
  ...
  const _NoPaymentLine({required this.compact});
